...
router
  .route('/:id')
  .get(getTaskById)
  .put(checkRole(["tl", "admin"]), updateTask)
  .delete(checkRole(["admin", "tl"]), deleteTask);

// Removed duplicate routes and kept one definition for assigning/updating
...
router
  .route('/:id/update')
  .post(checkRole("intern"), createUpdate)
  .get(listUpdates);
